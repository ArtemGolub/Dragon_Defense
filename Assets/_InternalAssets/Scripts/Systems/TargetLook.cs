...
public static class TargetLook
{
    ...
    public static void LookAtNewTarget(Transform looker, Transform target)
    {
        Vector3 lookPos = target.position - looker.transform.position;
        lookPos.y = 0;
        Quaternion rotation = Quaternion.LookRotation(lookPos);
        ...
        Quaternion newRotation = Quaternion.Slerp(looker.transform.rotation, yRotation,   Time.deltaTime * 5);
        looker.transform.rotation = newRotation;
    }
}
